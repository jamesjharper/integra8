...
impl<'a> ResultsNode<'a> {
    ...
    pub fn children(&self, detail_level: &DetailLevel) -> Vec<&ResultsNode<'a>> {
        match detail_level {
            ...
            DetailLevel::Error | DetailLevel::Warning => self
                .children
                .iter()
                .filter(|c| c.report.result.has_failed() || c.report.result.has_warn())
                .collect(),
            _ => self.children.iter().collect(),
        }
    }
    ...
    pub fn render_node<W: Write>(
        ...
        crate::render::render_component_heading(
            output_formatter,
            &self.report,
            style,
            detail_level,
        ...
        let render_attributes = match &self.report.result {
            ...
            ComponentResult::Pass(_) => detail_level == &DetailLevel::Verbose,
            ComponentResult::DidNotRun(_) => false,
        };

        if render_attributes {
            crate::render::render_node_attributes(
                output_formatter,
                &self.report,
                style,
                detail_level,
