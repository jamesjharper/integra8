...
use syn::{Token, Attribute, Meta, MetaNameValue, Lit, Path, Result};
...
pub fn try_parse_duration(attr: &Attribute, attr_name: &'static str) -> Result<Option<Duration>>  {
    ...
    match attr.parse_meta()? {
        Meta::NameValue(MetaNameValue { lit: Lit::Str(lit_str), .. }) => {
            let duration = humantime::parse_duration(&lit_str.value())
                .map_err(|e| Error::new_spanned(
                    attr, 
                    format!("Unable to parse duration string \"{}\",  {:?}", lit_str.value(), e.to_string()))
                )?;
            Ok(Some(duration))
        }
        _ => {
            Err(Error::new_spanned(attr, format!("expected #[{} = \"...\"]", attr_name)))
        }
    }  
}
