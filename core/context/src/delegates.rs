...
pub enum Delegate<TParameters> {
    ...
    AsyncWithContext(fn(ExecutionContext<TParameters>) -> Pin<Box<dyn Future<Output = ()> + Send + 'static>>)
}
...
impl<TParameters> Delegate<TParameters> {
    ...
    pub fn async_without_context(del: fn() -> Pin<Box<dyn Future<Output = ()> + Send + 'static>>) -> Self {
        Self::AsyncWithoutContext(del)
    }

    pub fn async_with_context(del: fn(ExecutionContext<TParameters>) -> Pin<Box<dyn Future<Output = ()> + Send + 'static>>) -> Self {
