...
use crate::decorations::SourceLocation;
        ...
        match self.result {
            ComponentResult::Pass(PassReason::Accepted) => {
                ...
                if self.timing.is_warn() {
                    ...
                    out.write_plain(format!(" - {} - (time limit warning) - {:?} ", self.display_name, self.timing.duration()))?;
                } else {
                    out.write_pretty(type_icon, term::color::GREEN)?;
                    out.write_plain(format!(" - {} - {:?} ", self.display_name, self.timing.duration()))?;
                }
            ...
            ComponentResult::Fail(FailureReason::Rejected) => {
                ...
                let _type_icon = match self.component_type {
                    ...
                    _ => {
                        ...
                        out.write_plain(format!("    - location: {}", self.src_location.hotlink_text()))?;
                        out.write_plain("\n")?;
            ...
            ComponentResult::Fail(FailureReason::Overtime)=> {
                out.write_pretty(type_icon, term::color::RED)?;
                out.write_plain(format!(" - {} - (time limit exceeded) - {:?} ", self.display_name, self.timing.duration()))?;
            },
...
impl<'a> TreeNodePrinter<'a> {
    ...
    pub fn print_node(&mut self, 
        ...
        node.render_node(prefix, next_prefix, &mut self.out).unwrap();
