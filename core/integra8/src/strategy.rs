...
use crate::components::{Component, RootSuite, IntoTaskStateMachine};
...
pub trait ResolveComponentsStrategy<Parameters> {
    ...
    fn create(parameters: &Parameters) -> Self where Self: Sized ;
    ...
    ///
    fn resolve_components(&mut self, parameters: &Parameters, auto_detected: Vec<ComponentDecoration<Parameters>>) -> Vec<ComponentDecoration<Parameters>>;
}
...
pub trait ResolveComponentScheduleStrategy<Parameters> {
    ...
    ///
    fn resolve_schedule(&mut self, parameters: &Parameters , components: Vec<ComponentDecoration<Parameters>>) -> TaskStateMachineNode<Component<Parameters>>;
}
...
pub trait ResolveRunnerStrategy<Parameters> {
    ...
    ///
    fn resolve_runner(&mut self, parameters: &Parameters, sender: ResultsSource) -> Box<dyn ScheduleRunner<Parameters>>;
}
...
impl<Parameters> ResolveComponentsStrategy<Parameters> for DefaultResolveComponentsStrategy {
    ...
    fn create(_parameters: &Parameters) -> Self where Self: Sized {
        Self
    }
    ...
    /// * `auto_detected` - The list of already automatically detected component decoration.
    ///
    fn resolve_components(&mut self,  _parameters: &Parameters, auto_detected: Vec<ComponentDecoration<Parameters>>) -> Vec<ComponentDecoration<Parameters>> {
        auto_detected
    }
} 
...
impl<Parameters: TestParameters> ResolveComponentScheduleStrategy<Parameters> for DefaultResolveComponentScheduleStrategy {
    ...
    ///
    fn resolve_schedule(&mut self, parameters: &Parameters, components: Vec<ComponentDecoration<Parameters>>) -> TaskStateMachineNode<Component<Parameters>> {
        RootSuite::from_decorated_components(
            components,
            parameters
        ).into_task_state_machine()
    }
}
...
impl<Parameters: FormatterParameters  + TestParameters> ResolveFormatterStrategy<Parameters> for DefaultResolveFormatterStrategy {
...
impl<Parameters: TestParameters + Sync + Send + UnwindSafe + 'static> ResolveRunnerStrategy<Parameters> for DefaultResolveRunnerStrategy {

    fn create(_parameters: &Parameters) -> Self where Self: Sized  {
        Self
    }

    fn resolve_runner(&mut self, _parameters: &Parameters, sender: ResultsSource) -> Box<dyn ScheduleRunner<Parameters>> {
        Box::new(
...
pub trait TestApplicationLocator<Parameters> {
    ...
    fn resolve_components_strategy(parameters: &Parameters) -> Box<dyn ResolveComponentsStrategy<Parameters>>;

    fn resolve_component_schedule_strategy(parameters: &Parameters) -> Box<dyn ResolveComponentScheduleStrategy<Parameters>>;

    fn resolve_formatter_strategy(parameters: &Parameters) -> Box<dyn ResolveFormatterStrategy<Parameters>>;

    fn resolve_runner_strategy(parameters: &Parameters) -> Box<dyn ResolveRunnerStrategy<Parameters>>;
}
