...
use crate::decorations::{TestDecoration, ConcurrencyMode, TestAttributesDecoration};
...
impl TestAttributes {
    pub fn new<TParameters: TestParameters>(
        ...
        Self {
            identity: ComponentIdentity::new(
                def.name,
                def.path
            ),
            ...
            warn_threshold: def.warn_threshold
                .map_or_else(
                    || parent_desc.test_warn_threshold,
                    |val| val
                ),

            critical_threshold: def.critical_threshold
                .map_or_else(
                    || parent_desc.test_critical_threshold,
                    |val| val
                ),

            concurrency_mode: def.concurrency_mode
                .map_or_else(
                    || parent_desc
...
mod test_async_impl {
    ...
    pub struct TestFnAsync<TParameters> {
        pub test_fn: fn(ExecutionContext<TParameters>) -> Pin<Box<dyn Future<Output = ()> + Send + 'static>>
    }
