...
use crate::scheduling::state_machine::{TaskStateMachineNode, SerialTaskNode, ParallelTaskNode};

use crate::components::{Component, Suite, Test, SuiteState};
...
impl<TParameters: TestParameters> IntoTaskStateMachine<Component<TParameters>> for Suite<TParameters> {
    ...
    fn into_task_state_machine(mut self) -> TaskStateMachineNode<Component<TParameters>> {
        ...
        root_node.enqueue(
            Component::Suite(self.description.clone(), self.attributes.clone(),SuiteState::Start)
        );
        ...
        root_node.enqueue(
            self.tests.into_task_state_machine()
        );
        ...
        self.suites.drain(..)
            .for_each(|suite| {
                match suite.attributes.suite_concurrency_mode {
        ...
        root_node.enqueue(
            Component::Suite(self.description, self.attributes, SuiteState::Finish)
        );
...
impl<TParameters: TestParameters> IntoTaskStateMachine<Component<TParameters>> for Vec<Test<TParameters>> {
    
    fn into_task_state_machine(self) -> TaskStateMachineNode<Component<TParameters>> {
        TestIntoComponentTaskStepIterator::new(self)
            .fold(SerialTaskNode::new(),
            |mut seq, node| {
                seq.enqueue(node);
                seq
            }).into()
...
impl<TParameters: TestParameters>  Iterator for TestIntoComponentTaskStepIterator<TParameters>  {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while let Some(next) = self.test_iter.next_if(|x| x.attributes.concurrency_mode == ConcurrencyMode::Parallel) {
            parallelizable_group.append(Component::Test(next));
        }
