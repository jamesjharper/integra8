...
use crate::results::{DidNotRunReason, FailureReason, PassReason, ComponentRunReport};
use crate::results::summary::{PassResultsCountSummary, FailResultsCountSummary, DidNotRunResultsCountSummary};
use crate::results::summary::counts::ResultReasonCounter;

pub use std::slice::Iter;

/// A struct for interrogating *pass* results.
/// implements `Iterator` and can be projected to narrower results set using its accompanying  `due_to...` methods 
...
impl<'a> PassedResults<'a> {
    ...
    pub fn from(
        ...
        counts: &'a PassResultsCountSummary
    ) -> Self {
        Self {
    ...
    /// Returns a iterator of only the pass results which matches the give pass reason
    /// 
    /// # Arguments
    ///
...
impl<'a> Iterator for PassReasonResults<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.iter.find_map(|report| {
            match &report.result {
                ComponentResult::Pass(reason) if reason == &filter_by_reason => {
                    Some(report)
                },
                _ => None
            }
...
/// A struct for interrogating *fail* results.
/// implements `Iterator` and can be projected to narrower results set using its accompanying  `due_to...` methods 
...
impl<'a> FailedResults<'a> {
    ...
    pub fn from(
        iter: Iter<'a, ComponentRunReport>,
        counts: &'a FailResultsCountSummary
    ) -> Self {
        Self {
            iter: ChainedResultsIter::from_single(iter, counts)
        }
...
impl<'a> Iterator for FailedReasonResults<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.iter.find_map(|report| {
            match &report.result {
                ComponentResult::Fail(reason) if reason == &filter_by_reason => {
                    Some(report)
                },
                _ => None
            }
...
impl<'a> Iterator for NotRunReasonResults<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.iter.find_map(|report| {
            match &report.result {
                ComponentResult::DidNotRun(reason) if reason == &filter_by_reason => {
                    Some(report)
                },
                _ => None
            }
...
impl<'a, ResultsCountSummary: ResultReasonCounter> ChainedResultsIter<'a, ResultsCountSummary> {
    ...
    pub fn from_single(iter: Iter<'a, ComponentRunReport>, result: &'a ResultsCountSummary) -> Self {
        Self {
    ...
    pub fn count(&self) -> usize {
        self.iters.iter().fold(0, |acc, (_, counts)| counts.total() + acc)
    }

    pub fn count_due_to_reason(&self, reason: &<ResultsCountSummary as ResultReasonCounter>::ReasonType) -> usize {
        self.iters.iter().fold(0, |acc, (_, counts)| counts.by_reason(&reason) + acc)
    }
}
