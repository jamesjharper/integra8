...
use crate::scheduling::TaskStream;
...
impl<In, Out, Stream> TaskStreamMap<In, Out> for Stream where Stream: TaskStream<Payload = In> {  
    fn map<F>(self, f: F) -> iter::TaskStreamMap<Self, F>
    where
        Self: Sized + TaskStream<Payload = In>,
        F: FnMut(In) -> Out {
...
mod iter {
    use crate::scheduling::{TaskStream, PollTaskResult, TaskNodePath};
