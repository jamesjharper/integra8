...
use crate::results::{ComponentTimeResult, ComponentRunReport};
    ...
    fn process_message(&mut self, msg : TestEvent) -> bool {
        match msg {
            ...
            TestEvent::NotifyRunStart { 
                test_count, suite_count, tear_down_count, setup_count
            } => self.sink.on_run_start(test_count, suite_count, tear_down_count, setup_count),

            TestEvent::NotifyComponentStart {
                description
            } => self.sink.on_component_start(description),

            TestEvent::NotifyComponentTimeout {
                description, timing_result
            } => self.sink.on_component_timed_out(description, timing_result),

            TestEvent::NotifyComponentComplete {
                report
...
impl ResultsOutputWriterSink  {
    ...
    fn on_component_start(
        ...
        match description.component_type {
            ComponentType::Suite => {
                self.output_writer.write_suite_start(&description)
            },
            ComponentType::Test => {
                self.output_writer.write_test_start(&description)
            },
            ComponentType::Setup => {
                self.output_writer.write_setup_start(&description)
            },
            ComponentType::TearDown => {
                self.output_writer.write_tear_down_start(&description)
            },
        }?;
    ...
    pub fn on_component_timed_out(
        ...
        match description.component_type {
            ComponentType::Suite => {
                self.output_writer.write_suite_timeout(&description, &timing_result)
            },
            ComponentType::Test => {
                self.output_writer.write_test_timeout(&description, &timing_result)
            },
            ComponentType::Setup => {
                self.output_writer.write_setup_timeout(&description, &timing_result)
            },
            ComponentType::TearDown => {
                self.output_writer.write_tear_down_timeout(&description, &timing_result)
            },
        }?;
        self.output_writer.write_component_timeout(&description, &timing_result)?;
        Ok(())
    }
    ...
    pub fn on_component_complete(
        ...
        let result = match report.description.component_type {
            ComponentType::Suite => {
                self.output_writer.write_suite_report(&report)
            },
            ComponentType::Test => {
                self.output_writer.write_test_report(&report)
            },
            ComponentType::Setup => {
                self.output_writer.write_setup_report(&report)
            },
            ComponentType::TearDown => {
                self.output_writer.write_tear_down_report(&report)
            },
        }.and_then(|_| {
            self.output_writer.write_component_report(&report)
        });
