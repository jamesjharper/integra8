...
use types::{Parameter, StructoptStruct, OutputFormatterType};
use types::{ParameterValue, StructoptStructValue, StringParameterValue, MainDefinitionValue, OutputFormatterTypeValue};
...
impl Parse for ApplicationParameters {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let attrs = input.call(Punctuated::<Parameter, Token![,]>::parse_terminated)?
          .into_iter()
          .map(|i| (i.key, i.value)).collect::<HashMap<String, ParameterValue>>();
...
impl ApplicationParameters {    
    ...
    pub fn take_console_output_formatter(&mut self) -> OutputFormatterType {
        ...
        .unwrap_or_else(|| {
            OutputFormatterTypeValue::InlineFactoryType {
                formatter_factory_type: parse_quote!(::integra8::formaters::pretty::PrettyFormatter)
            }
        }).render_tokens()
    }

    fn take_output_formatter_type(&mut self, name : &'static str) -> Option<OutputFormatterTypeValue> {
        self.take(name)
            .map(|attr| {
                match attr {
        ...
        self.take(name)
            .map(|attr| {
                match attr {
                    ...
                    _ => abort!("expected felid `{}` to resolve to a body of a struct, or a type", name)
                }
            })
    }
